...
The ICO or ICNS inventory remains available in validation and import results and is persisted on the physical object by SQLite schema v3; the library format remains version 1. `encoded_size` counts all encoded payload bytes required by a logical representation: the PNG or complete DIB payload for ICO; the graphical element payload for standalone ICNS; both color and required 8-bit mask payloads for legacy ICNS RGB pairs. The mask has no separate representation, and the ordinal remains that of the physical color element. The database stores no payload offsets, mask references, FourCC values, unknown elements, parser details, decoded data, or warnings. Precise payload access requires reparsing and revalidating the immutable object.
