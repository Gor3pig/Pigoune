...
Le pipeline suit cet ordre : validation des arguments et extraction des octets POSIX du seul nom de fichier → copie dans `objects/.tmp` et SHA-256 → validation Glycin et analyse SVG depuis le staging → recherche des assets logiques par hash en mode `Detect` → publication durable de l'objet → horodatage UTC et nouvel `AssetId` → transaction SQLite. Après le staging, le fichier source n'est plus lu ; aucun chemin source n'est enregistré. Les octets publiés restent identiques aux octets copiés.
